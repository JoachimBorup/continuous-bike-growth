def analyze_existing_infrastructure(placeids:list):
...
def analyze_poi_base_results_iterations(placeid:str, subgraphs_percentages:list, iterations:int):
    ...
    for subgraph_percentage in tqdm(subgraphs_percentages, desc="Subgraphs percentages list", leave=False):
        for i in tqdm(range(iterations), desc="Iterations", leave=False):
            ...
            resultfile = open(PATH["results"] + placeid + "/" + subgraph_percentage + "/" + str(i) + "/" + filename + ".pickle",'rb')
